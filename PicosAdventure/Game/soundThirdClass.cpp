...
bool SoundThirdClass::setup(HWND hwnd)
{
	...
	result = loadFile("laser_continu_01", "laser_continu_01");
	if(!result)
	{
		MessageBoxA(NULL, "Could not load laser_continu_01", "SoundSecondClass - Error", MB_ICONERROR | MB_OK);
		return false;
	}
	result = loadFile("laser_continu_02", "laser_continu_02");
	if(!result)
	{
		MessageBoxA(NULL, "Could not load laser_continu_02", "SoundSecondClass - Error", MB_ICONERROR | MB_OK);
		return false;
	}
